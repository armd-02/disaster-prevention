...
class poiMarker {
	...
	static select(poiid, detail) {								// Map move to PoiId & Zoom(config)
		return new Promise((resolve, reject) => {
			...
			let zoomlv = Conf.act_PoiViewZoom >= map.getZoom() ? Conf.act_PoiViewZoom : map.getZoom();
			...
			switch (target) {
				...
				case "activity":	// アクティビティリストのカテゴリを表示
					...
					for (const act of poiCont.pois().acts) {
						acts.set(act.category, true); // 同じキーに何度も値を設定しても問題ない
					};
